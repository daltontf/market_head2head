...
#  "CAR": (["Carolina"],[]),  #More of designation than market since it is Raleigh and Charlotte
  ...
  "DEN": (["Denver", "Colorado"],[]),
  "DET": (["Detroit", "Michigan Panthers"],["Michigan K-Wings"]),
  "EDM": (["Edmonton"],[]),
...
for team1, team1_clause in team_clauses.items():
  if len(args) > 1 and team1 != args[1]: continue
  for team2, team2_clause in team_clauses.items():
    if len(args) > 2 and team2 != args[2]: continue
    if team1 == team2: continue
    if len(args) == 1 and team1 > team2: continue
    clauses = list(itertools.product(team1_clause[0], team2_clause[0]))

    team1_home_clauses = " or ".join(list(map(lambda x: f"title like '%{x[1]}%at%{x[0]}%' or title like '%{x[0]}%vs%{x[1]}%'" , clauses)))
    team2_home_clauses = " or ".join(list(map(lambda x: f"title like '%{x[0]}%at%{x[1]}%' or title like '%{x[1]}%vs%{x[0]}%'" , clauses)))

    
    if team1_clause[1]:
        team1_home_clauses = "(" + team1_home_clauses + ") and (" + " or ".join(list(map(lambda x: f"title not like '%at%{x}%' or title not like '%{x}%vs%'" , team1_clause[1]))) + ")"
        team2_home_clauses = "(" + team2_home_clauses + ") and (" + " or ".join(list(map(lambda x: f"title not like '%{x}%at%' or title not like 'vs%{x}%'" , team1_clause[1]))) + ")"

    if team2_clause[1]:
        team2_home_clauses = "(" + team2_home_clauses + ") and (" + " or ".join(list(map(lambda x: f"title not like '%at%{x}%' or title not like '%{x}%vs%'" , team2_clause[1]))) + ")"
        team1_home_clauses = "(" + team1_home_clauses + ") and (" + " or ".join(list(map(lambda x: f"title not like '%{x}%at%' or title not like 'vs%{x}%'" , team2_clause[1]))) + ")"
...
.print "<table><caption>{team1} - {team2} by Matchup:</caption>" """)
      ...
      print(f".print {team1} - {team2} by Matchup:")  
    print("select * from temp_table;")
    if mode == 'html':
      print(".print </table>")
      print(f""".print "<table><caption>{team1} - {team2} by League:</caption>" """)
    else:
      print(f".print {team1} - {team2} by League:")    
    print(f"""  
select League, 
...
from temp_table group by League;""")
    if mode == 'html':
      print(".print </table>")
      print(f""".print "<table><caption>{team1} - {team2} Total:</caption>" """)
    else:
      print(f".print {team1} - {team2} Total:")     
    print(f"""  
select 
    sum("{team1} Wins") as '{team1} Wins',
    sum("{team2} Wins") as '{team2} Wins',
